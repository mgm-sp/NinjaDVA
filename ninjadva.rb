	...
	def initialize(config, options = {})
	  # set necessary variables
	   dashboard_dir = "./dashboard_vm/"
		...
		# check whether we are controlled by ninjaDVA Vagrant Manager
		if Dir.exists?("./dashboard_vm/")
		  require 'fileutils'

		  # copy tasks to dashboard, before the vm is started
		  config.trigger.before :up do
			  yaml_list = Dir.glob("./" + $current_vm_path + "/"+ options[:challenge_descriptions_dir] + "/*.yaml")
		...
		# delete all challanges  included in this vm from the dashboard vm
	     config.trigger.after :halt do
			...
			Dir.glob("./" + $current_vm_path + "/" + options[:challenge_descriptions_dir] +"/*.yaml").each{|file|
